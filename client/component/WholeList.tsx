...
const WholeList = (props: dataType) => {
    ...
    return (<>
        {wholeGoups.map((group, groupIdx) => {
            ...
            let activedListClass = openGroup ? "activedList" : "";
            return (
                <div key={groupIdx} className="pt-3.5">
                    {header != 'Default' && <h3 className={"text-xl hover:cursor-pointer flex justify-between justify-items-center " + activedListClass}
                        onClick={() => setOpenGroupArray([groupIdx, !openGroup])} id={header}>
                        <div >{header}</div>
